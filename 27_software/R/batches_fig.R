...
for(i in 1:5) {
    ...
    pdf(paste0("../Figs/batches_fig", i, ".pdf"), height=5, width=10, pointsize=28)
    par(bg=bgcolor, mar=rep(0.1, 4), bty="n")
    ...
    text(x[1]+2, y[1]-5, "genotypes", col=bgcolor, adj=c(0, 0))
    text(x[2]+2, y[1]-5, "phenotypes", col=bgcolor, adj=c(0, 0))

    dev.off()
}


pdf("../Figs/batches_fig6.pdf", height=5, width=10, pointsize=28)
par(bg=bgcolor, mar=rep(0.1, 4), bty="n")
...
pdf("../Figs/batches_fig7.pdf", height=5, width=10, pointsize=28)
par(bg=bgcolor, mar=rep(0.1, 4), bty="n")
