...
\note{I taught part of a course on statistical programming at Johns
  ...
  Most people recommend indenting by 4 spaces. I used to use 2 spaces,
  but I've now switched to 4. (4 spaces are easier to track.)
...
\item Give details
  \bi
  \item {\ttfn {\hilit glue::glue(}"nrow(X) ({\hilit \{nrX\}}) != nrow(Y) ({\hilit \{nrY\}})"{\hilit )}}
  \ei
...
\item Bugs increase with program length; there is a big jump between
  one screen length vs. longer.
\ei
...
\note{R packages are great for encapsulating and distributing R code
  ...
  We'll talk about writing R packages in a few weeks. It's really rather
  easy.
...
\note{Complex data objects allow you to think about complex data as a
  ...
  R has three systems for object-oriented programming: S3, S4, and R6. They
