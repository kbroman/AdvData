...
# effectplot.R
#
# copyright (c) 2002-7, Hao Wu, The Jackson Laboratory
...
function (cross, pheno.col = 1, mname1, mark1, geno1, mname2, 
          ...
          legend.lab, lty, draw=TRUE, var.flag=c("pooled","group")) 
{
...
#      }
      ...
      if(add.legend) {
        ...
        y.leg <- a[4] - diff(a[3:4]) * 0.10
        y.leg2 <- a[4] - diff(a[3:4]) * 0.05
        legend(x.leg, y.leg, geno1, lty = lty, pch = 1, col = col, 
               cex = 1, xjust = 0.5)
