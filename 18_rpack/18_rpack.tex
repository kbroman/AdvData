...
\note{R packages can be big and important.
     ...
     Moreover, the introduction of the R package {\hilit devtools}
     including the function {\tt install\_github()}, makes it even
     easier to share your R package with folks.
}
\end{frame}
...
\note{The Writing R Extensions manual is the key source for the
  ...
  But you can delay reading this for quite some time. Put it aside and
...
\hfill {\tt \footnotesize \lolit
  \href{http://alyssafrazee.com/2014/03/06/RSkittleBrewer.html}{alyssafrazee.com/2014/03/06/RSkittleBrewer.html}}


\note{
  Alyssa Frazee wrote a small R package with color schemes based on a
  common type of candy in the US. Her package is a perfect little example
...
\hfill {\tt \footnotesize \lolit
  \href{https://github.com/alyssafrazee/RSkittleBrewer}{github.com/alyssafrazee/RSkittleBrewer}}
...
\note{
  ...
  There may be other things, but these are the only critical ones.
}
\end{frame}
...
\note{
  ...
  One way to create this sort of file is to copy and edit one
  from some other package.

  One part that might be unclear is the {\tt License} field. You
  ...
  LICENSE} file or R will complain. You could copy and edit the one from the
  RSkittleBrewer package.

  Imports and Suggests indicate which packages this package depends
  on. VignetteBuilder to include an R Markdown based tutorials.
  RoxygenNote gets added automatically by the documentation package.
}
\end{frame}
...
automates things otherwise hard to remember

\vspace{12pt}

\bi
\item {\tt create\_package()}
\item {\tt use\_git()}, {\tt use\_github()}
\item {\tt use\_mit\_license()}, {\tt use\_gpl3\_license()}
\item {\tt use\_readme\_md()}
\item {\tt use\_r("function\_name")}
\item {\tt use\_roxygen\_md()}
\item {\tt use\_package("stats", "imports")}
\item {\tt use\_vignette()}
\ei


\note{
  The usethis package can be super helpful as it automates lots of
  stuff where the details can be tricky and hard to remember.
}
\end{frame}
...
\item Use href{https://usethis.r-lib.org}{usethis} package to assist
  with all of the little nit-picky package development things.
\ei
